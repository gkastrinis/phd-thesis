...
\begin{figure}[htb]
\centering
\includegraphics[scale=.7]{assets/chart-soundness}
...
% A graph portraying how different kinds of static program analyses relate to each other and to the mathematical ideals might be of interest. Suppose that each set of program behaviors (examined by the given analysis) is mapped to some integer value. Indeed, such a mapping is possible since the set of all potential behaviors is \emph{countably infinite} (?) and any partial order on its subsets can allow a mapping to integers. Those integer values bear no actual semantics; they are just an easy way to \emph{visualize} different subsets. Under that assumption, figure \ref{fig:soundness} makes such an attempt. The x-axis represents different program inputs (i.e., different program executions), and the y-axis represents the exhibited behaviors in each execution. All static program analyses make claims regarding program behaviors in all potential executions, and thus their results are visualized as a constant straight line.
