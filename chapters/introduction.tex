...
% Ancestor(x, y) :- |$\forall$| Parent(x, z), Ancestor(z, y).
% \end{datalog}

% \begin{minted}[escapeinside=||,mathescape=true]{prolog}
% Ancestor(x, y) :- Parent(x, z), Ancestor(z, y),
% %$\forall$
% Parent(x, z),
% Parent(x, z).
...
\rel{Ancestor}{x, y} $\leftarrow$ \rel{Parent}{x, y}.

\rel{Ancestor}{x, y} $\leftarrow$ \rel{Parent}{x, z}, \rel{Ancestor}{z, y}.
