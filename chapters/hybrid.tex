...
%, before describing related work (Section~\ref{sec:related}).
...
literature \cite{uss:2009:Guarnieri,fse:2013:Madsen},
...
\cline{1-1}
%\begin{tabular}{ll}
%\rel{Alloc}{var : V, heap : H, inMeth : M}   &   \rel{FormalArg}{meth : M, i : $\mathbb{N}$, arg : V} \\ 
%\rel{Move}{to : V, from : V}               &   \rel{ActualArg}{invo : I, i : $\mathbb{N}$, arg : V} \\ 
%\rel{Load}{to : V, base : V, fld : F}      &   \rel{FormalReturn}{meth : M, ret : V} \\                
%\rel{Store}{base : V, fld : F, from : V}   &   \rel{ActualReturn}{invo : I, var : V} \\                
%\rel{VCall}{base : V, sig : S, invo : I, inMeth : M}   &   \rel{ThisVar}{meth : M, this : V} \\                    
%\rel{SCall}{meth : M, invo : I, inMeth : M}&   \rel{HeapType}{heap : H, type : T} \\
%                                            &   \rel{LookUp}{type : T, sig : S, meth : M} \\            
%\rel{InMethod}{instr : I, meth : M}        &   \rel{Subtype}{type : T, superT : T} \\
%\end{tabular}
\hspace{-3mm}
\begin{tabular}{l l}
...
\rel{SCall}{meth : M, invo : I, inMeth : M} & \args{\# Class.meth(..)}\\
\end{tabular}\\
\\
...
\item The input relations correspond to the intermediate language for
  ...
  information. For instance, the \relname{Alloc} relation represents
  ...
  input relations for all other instruction types (\relname{Move},
  \relname{Load}, \relname{Store}, \relname{VCall}, and
  \relname{SCall}). Similarly, there are relations that encode
  pertinent symbol table information.  Most of these are
  self-explanatory but some deserve explanation. \relname{LookUp}
...
\item There are five output or intermediate computed relations
  (\relname{VarPointsTo}, $\ldots$, \relname{Reachable}). Every
  ...
  \relname{VarPointsTo} and \relname{CallGraph}, encoding our
  points-to and call-graph results. The \relname{VarPointsTo}
  relation links a variable (\args{var}) to a heap object
  (\args{heap}). Other intermediate relations (\relname{FldPointsTo},
  \relname{InterProcAssign}, \relname{Reachable}) correspond to
  standard concepts and are introduced for conciseness. For instance,
  \relname{InterProcAssign} (which encodes all parameter and return
...
\item The base rules are not concerned with what kind of
  ...
  \relname{Alloc}) is analyzed. Thus, \consname{Record} is only used
...
\item The rules of Figure~\ref{fig:baserules} show how each input
  ...
  \relname{VCall}). The rule says that if a reachable method of the
