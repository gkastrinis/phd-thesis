...
%% For instance, the \predname{Move}
%% relation represents instructions that assign a local variable to another. There
%% are similar input relations for other instruction types (\predname{Load},
%% \predname{Store}, and \predname{Call}, for reading/writing heap object fields
%% and for virtual calls, respectively).
In particular, the \predname{Phi} relation captures
$\phi$ instructions, for the SSA form of our intermediate language. The
\predname{Next} relation expresses directed edges in the control-flow graph
(CFG): \pred{Next}{i,j} means that \args{i} is a CFG predecessor of \args{j}.

Similarly, there are relations that encode type system, symbol table, and
program environment information---e.g., \predname{FormalArg}, \predname{ActualArg},
\predname{FormalRet}, \predname{ThisVar}. 
%% %% These are mostly straightforward. 
%% For instance, \predname{FormalArg} shows which variable is a formal argument of
...
%% \predname{ActualArg} is
%% similar, but at a method invocation site.  \predname{FormalRet} combines
...
%% \predname{ThisVar} returns the \code{this} variable of a method.
...
type.  \predname{InMethod} is a function from instructions to their
containing methods. \predname{Resolved} is a predicate that can be
...
\predname{RootMethod} is a predicate over methods, used to start
...
analysis. The first relation, \predname{MustAlias}, is also the main output of
the analysis. The relation is defined on access paths, i.e., expressions of the
form ``\args{var}(\args{.fld})*''.  The meaning of \pred{MustAlias}{i, ctx,
...
\predname{MustCallGraphEdge} holds information for fully-resolved virtual
calls: invocation site \args{invo} will call method \args{toMth} under the
given contexts.  \predname{Reachable} computes which methods and under what
...
%% be used to configure the must-alias analysis.  Predicates \predname{Resolved},
%% \predname{MayAlias}, and \predname{CallMayStoreToField} are expected to be
%% computed by a \emph{may-point-to} analysis, running as a preprocessing step.
%% \predname{Resolved} holds variables that are determined to only point to
%% objects with a unique dynamic type. \predname{MayAlias} reflects whether two
%% variables (in the same method) may point to the same object.
%% \predname{CallMayStoreToField} does a transitive search of all methods that may
...
\item We use the shorthand \predname{P*} for the reflexive, symmetric,
	transitive closure of relation \predname{P}, which is assumed to be binary.
	...
	Specifically, \pred{MustAlias*}{i, ctx, \_, \_} denotes the reflexive,
	symmetric, transitive closure of relation \predname{MustAlias} with respect
...
% In practice, this is not a restriction:
	...
	``$\forall$\args{i}: \pred{P}{i} \rightrulearrow{} \pred{Q}{i,...}''  is
	true if \pred{Q}{i,...} holds for all \args{i} such that \pred{P}{i} holds.
	...
	``$\forall$\args{i,j}: \pred{P}{i,j,k} \rightrulearrow{}
	\pred{Q}{i,j,k,l}'' is interpreted as ``there exist \args{k,l} such that
	for all \args{i,j} ...''.
\end{itemize}
...
\begin{tabular}{l}
%\cline{1-1}
\pred{Reachable}{ctx,m} \rulearrow{} \predMay{RootMethod}{m}, \args{ctx} = \consname{All}.\\
\rule{-2pt}{3ex}
...
\pred{MustAlias}{i, ctx, ap, \consname{AP}(\args{to})} \rulearrow{} \\
...
\pred{MustAlias}{i, ctx, \funcapp{AP}{to}, \consname{AP}(\args{base.fld})} \rulearrow{} \\
\tab \pred{Load}{i, to, base, fld}, \\
\tab \pred{InMethod}{i, m}, \pred{Reachable}{ctx,m}. \\
\rule{-2pt}{3ex}

\pred{MustAlias}{i, ctx, \funcapp{AP}{from}, \consname{AP}(\args{base.fld})} \rulearrow{} \\
...
\tab \tab \pred{MustAlias}{reti, \consname{All}, calleeAp1, calleeAp2}), \\
...
%\pred{MustAlias}{i, ctx, ap1, ap2} \rulearrow{} \\
%\tab \pred{MustAlias}{i, \consname{All}, ap1, ap2}, \pred{InMethod}{i, meth}, \pred{Reachable}{ctx, meth}. \\
%\rule{-2pt}{3ex}
...
The former rule employs configuration predicate \predname{RootMethod}. This
...
The next four \predname{MustAlias} rules handle one instruction kind each:
\predname{Move}, \predname{Phi}, \predname{Load}, and \predname{Store}. The
\predname{Move} rule merely establishes an aliasing relationship between the
two assigned variables, at the point of the move instruction. The
\predname{Phi} rule promotes aliasing relationships that hold for all the
right-hand sides of a $\phi$ instruction to its left hand side. The
\predname{Load} and \predname{Store} rules establish aliases between the
loaded/stored expression, \args{base.fld}, and the local variable used.  The
last \predname{MustAlias} rule makes the \predname{MustAlias} relation
...
$\forall$\args{k} \rightrulearrow{} !\pred{Next}{k, firstInstr}.  This
...
become stronger. \predname{MustAlias} facts that were inferred to hold under
...
%% \predname{CallMayStoreToField}, supplied by a prior may-analysis.
...
%% In the above, \predname{MustAlias} inferences from (all predecessors
...
The analysis then propagates \predname{MustAlias} pairs from (all
...
The requirement that \funcapp{NewContext}{invo,ctx} produce contexts that
...
\predname{MustAlias} encodes equivalence classes on access paths. Datalog
...
%%\predname{MustAlias} is reflexively, symmetrically, and transitively
%%closed:
%%
%%\begin{small}
...
%%
%%
%%Since \predname{MustAlias} is an equivalence relation, it induces a
...
%%previous graph will get propagated. Subsequently, the \predname{Store}
...
%%\end{figure}
%%
%%A subsequent \predname{Load} operation, \code{z =
