...
%%   realistic programs}~\cite{soundiness15}. For instance, no realistic
...
\item We offer a general static may-point-to analysis that
  ...
  %% \cite{ecoop/HirzelDH04,toplas/HirzelDDH07,DSA:PLDI07} achieve
...
\stdparagraph{Baseline intra-procedural resoning.}
...
\stdparagraph{Inter-procedural elements.}
...
%\citet{Sreedhar:2000:FIO:349299.349326}
...
\stdparagraph{Control-flow merging.}
...
\stdparagraph{Heap manipulation.}
...
%% \pred{Reachable}{\consname{All}, meth} \rulearrow{} \predConf{RootMethod}{meth}.\\
%% \\
%% \pred{PointsTo}{i, ctx, \funcapp{AP}{var}, heap} \rulearrow{} \\
%% \tab \pred{Alloc}{i, var, heap}, \pred{InMethod}{i, meth}, \\
%% \tab \pred{Reachable}{ctx, meth}. \\
%% \\
%% \pred{PointsTo}{i, ctx, toAp, heap} \rulearrow{} \\
%% \tab \pred{Move}{i, to, from}, \\
%% \tab \pred{Before:PointsTo}{i, ctx, fromAp, heap}, \\
%% \tab \func{RebaseAP}{fromAp, from, to}{toAp}.\\
%% \\
%% \pred{CallGraphEdge}{i, ctx, toMeth, toCtx}, \\
%% \pred{Reachable}{toCtx, toMeth} \rulearrow{} \\
%% \tab \pred{Call}{i, base, sig}, \\
%% \tab \pred{Before:PointsTo}{i, ctx, \funcapp{AP}{base}, heap}, \\
%% \tab \pred{HeapType}{heap, type}, \pred{LookUp}{type, sig, toMeth},\\
...
%% %\hhline{=}
%% \\
%% \pred{PointsTo}{i, ctx, \funcapp{AP}{to}, heap} \rulearrow{} \\
%% \tab \pred{Load}{i, to, base, fld}, \\
%% \tab \pred{Before:PointsTo}{i, ctx, \funcapp{AP}{base.fld}, heap}.\\
%% \\
%% \pred{PointsTo}{i, ctx, \funcapp{AP}{base.fld}, heap} \rulearrow{} \\
%% \tab \pred{Store}{i, base, fld, from}, \\
%% \tab \pred{Before:PointsTo}{i, ctx, \funcapp{AP}{from}, heap}.\\
%% \\
%% \pred{PointsTo}{i, ctx, ap, heap1},\\
%% \pred{PointsTo}{i, ctx, ap, heap2} \rulearrow{} \\
%% \tab \pred{Store}{i, base, fld, from}, \\
%% \tab \pred{Before:PointsTo}{i, ctx, \funcapp{AP}{from}, heap1},\\
%% \tab \pred{Before:PointsTo}{i, ctx, ap, heap2},\\
%% \tab \args{ap} = \funcapp{AP}{base'.fld}, \args{base} $\neq$ \args{base'}.\\

%% \cline{1-1}
%% %\hhline{=}
%% \\
%% \pred{Before:PointsTo}{i, ctx, ap, heap} \rulearrow{} \\
%% \tab \pred{Next}{pred, i}, \\
%% \tab \pred{PointsTo}{pred, ctx, ap, heap},\\
%% \tab ($\forall$\args{j}: \pred{Next}{j, i} \rightrulearrow{} \pred{PointsTo}{j, ctx, ap, \_}).\\
%% \\
%% \pred{PointsTo}{i, ctx, ap, heap} \rulearrow{} \\
%% \tab \pred{Before:PointsTo}{i, ctx, ap, heap}, \args{ap} = \funcapp{AP}{var}, \\
%% \tab !\pred{Load}{i, var, \_, \_}, !\pred{Move}{i, var, \_}, \\
%% \tab !\pred{Alloc}{i, var, \_}, !\pred{Unknown}{i}.\\
%% \\
%% \pred{PointsTo}{i, ctx, ap, heap} \rulearrow{} \\
%% \tab \pred{Before:PointsTo}{i, ctx, ap, heap}, \args{ap} = \funcapp{AP}{var.\_}, \\
%% \tab !\pred{Call}{i, \_, \_}, !\pred{Store}{i, \_, \_, \_}, !\pred{Load}{i, var, \_, \_}, \\
%% \tab !\pred{Move}{i, var, \_}, !\pred{Alloc}{i, var, \_}, !\pred{Unknown}{i}.\\

%% \\
%% \pred{PointsTo}{i, ctx, ap, heap} \rulearrow{} \\
%% \tab \pred{Store}{i, \_, fld, \_}, \\
%% \tab \pred{Before:PointsTo}{i, ctx, ap, heap}, \\
%% \tab \args{ap} = \funcapp{AP}{\_.\_}, \args{ap} $\neq$ \funcapp{AP}{\_.fld.\_}. \\
%% \\
%% %\cline{1-1}
%% \end{tabular}
%% %\end{center}
...
%% \pred{Before:PointsTo}{fstInstr, toCtx, ap, heap} \rulearrow{} \\
%% \tab \pred{CallGraphEdge}{i, ctx, toMeth, toCtx}, \\
%% \tab \pred{InMethod}{fstInstr, toMeth}, 
%%      ($\forall$\args{k}: \rightrulearrow{} !\pred{Next}{k, fstInstr}), \\
%% \tab \pred{Before:PointsTo}{i, ctx, callerAp, heap}, \\
%% \tab \pred{FormalArg}{toMeth, n, toVar}, \pred{ActualArg}{i, n, var}, \\
%% \tab \func{RebaseAP}{callerAp, var, toVar}{ap}.\\
%% \\
%% \pred{RebaseAPForReturn}{i, ap, newAp} \rulearrow{} \\
%% \tab \pred{CallGraphEdge}{i, \_, toMeth, \_}, \\
%% \tab \pred{FormalArg}{toMeth, n, var}, \pred{ActualArg}{i, n, toVar}, \\
%% \tab \args{ap} = \funcapp{AP}{\_.\_}, \func{RebaseAP}{ap, var, toVar}{newAp}.\\
%% \\
%% \pred{PointsTo}{i, ctx, ap, heap} \rulearrow{} \\
%% \tab \pred{CallGraphEdge}{i, ctx, toMeth, toCtx}, \\
%% \tab \pred{Return}{ret}, \pred{InMethod}{ret, toMeth}, \\
%% \tab \pred{Before:PointsTo}{ret, toCtx, calleeAp, heap}, \\
%% \tab \pred{RebaseAPForReturn}{i, calleeAp, ap}, \\
%% \tab ($\forall$\args{toMeth'}, \args{toCtx'}: \\
%% \tab \tab \pred{CallGraphEdge}{i, ctx, toMeth', toCtx'} \rightrulearrow{} \\
%% \tab \tab ($\exists$\args{ret'}, \args{calleeAp'}: \\
%% \tab \tab \tab \pred{Return}{ret'}, \pred{InMethod}{ret',toMeth'},  \\
%% \tab \tab \tab \pred{Before:PointsTo}{ret', toCtx', calleeAp', \_}, \\
%% \tab \tab \tab \pred{RebaseAPForReturn}{i, calleeAp', ap})). \\
%% %\hhline{=}
%% %\cline{1-1}
%% \end{tabular}
%% %\end{center}
...
\stdparagraph{Alloc, Move, Load, Store-1.} The first four rules of the
...
\stdparagraph{Store-2.} The \textsc{Store-2} rule is the first one
...
\stdparagraph{Frame-1, Frame-2, Frame-3.}
...
%targets of a virtual call in relation \predname{CallGraphEdge}.
%(We
...
%%% SPACE
%% \stdparagraph{Stratification and two-stage analysis.}
%% The last rule shown in Section~\ref{sec:model} violates the
%% requirements of pure Datalog. It uses $\forall$ over the
%% \predname{CallGraphEdge} relation, even though the latter is being
%% computed simultaneously with the \predname{PointsTo} relation. (As
...
%% \predname{PointsTo} inference using this rule will not cause its own
%% invalidation, because it introduces
%% % (e.g., because of circular data flow in the program) 
%% a new \predname{CallGraphEdge} tuple that represents a call to a
...
%% \predname{CallGraphEdge} relation currently being computed, but one
...
%% \predname{CallGraphEdge} computed by the first run, only for this
...
\stdparagraph{Expanding the Analysis Reach.}
...
\begin{itemize}
\item 
%\stdparagraph{More liberal control-flow merging.}
...
%% \stdparagraph{Why access-path-points-to?}
...
%% analyses~\cite{popl/SmaragdakisBL11,pldi/KastrinisS13}.  However, as
...
%% %the \consname{Record} constructor of past work~\cite{popl/SmaragdakisBL11,pldi/KastrinisS13} 
%% on the rule for \predname{Alloc}. This addition is orthogonal to all
...
%    Type Analysis (RTA)} \cite{bacon-sweeney-oopsla96} and
%  \emph{Class Hierarchy Analysis (CHA)}
%  \cite{Dean:1995:OOP:646153.679523} are well-known conservative
...
%\citet{DBLP:conf/ictac/NikolicS12}.
...
%\stdparagraph{Scalability caveats.}
...
\stdparagraph{Coverage.}  Figure~\ref{fig:coverage} shows the coverage
...
\stdparagraph{Comparison with intra-procedural.}
...
\stdparagraph{Client analysis: devirtualization.}
...
\stdparagraph{Concurrency model.}
...
\stdparagraph{Points-to set sizes.}
...
%\citet{ecoop/HirzelDH04,toplas/HirzelDDH07}
...
%\citet{Sreedhar:2000:FIO:349299.349326}
Sreedhar et al. \cite{Sreedhar:2000:FIO:349299.349326} present the
...
%instance, for reflection analysis~\cite{aplas/Smaragdakis15}.) 
...
%% \stdparagraph{Initializing the analysis.}
...
%% rule. The input relation \predname{RootMethod} can contain arbitrary
%% contents and only affects the coverage of the analysis (since
%% otherwise points-to sets remain empty, i.e., implicitly
%% over-approximate).

%% \stdparagraph{Object allocation.} The 
%% handling of \predname{Alloc} instructions provides an unconditionally
...
%% \stdparagraph{Variable moves.} \predname{Move} instructions
...
%% set encoded in relation \predname{Before:PointsTo} (more precisely:
...
%% relation \predname{PointsTo}, for any \args{(i,ctx,toAp)} that match
...
%% \stdparagraph{Heap loads.} Similarly, heap loads produce over-approximate
...
%% \predname{Before:PointsTo} should be sound (over-approximate), i.e.,
...
%% \stdparagraph{Heap stores.} The next two rules handle writes to the heap
%% (store instructions).
...
%% \myparagraph{Assumption:} Before execution of the rule, the points-to
%% sets encoded in relation \predname{Before:PointsTo}, for any of the
...
%% \stdparagraph{Control-flow predecessors.} 
...
%% \myparagraph{Assumption:} Before execution of the rule, the points-to
%% sets encoded in relation \predname{PointsTo} should be sound, i.e.,
...
%% points-to sets (\predname{PointsTo} for \args{ap} at all predecessors)
...
%% \stdparagraph{Frame rules: before an instruction to after it.}
...
%% set for \args{ap} encoded in relation \predname{Before:PointsTo}
%% should be sound (over-approximate).

%% \myparagraph{Guarantee:} For each rule separately, the resulting
%% points-to set (in relation \predname{PointsTo}) is sound after full
...
%% \predname{Alloc}, \predname{Move} or \predname{Load}. (\predname{Call}
...
%% of \predname{Unknown} instructions is as conservative as it can
...
%% \stdparagraph{Inter-procedural analysis: Over-approximate call-graph.}
...
%% \myparagraph{Assumption:} Before execution of the rule, the points-to
%% set for \args{base} encoded in relation \predname{Before:PointsTo}
%% should be sound (over-approximate).

%% \myparagraph{Guarantee:} Relation \predname{CallGraphEdge} for
%% invocation site \args{i} encodes a sound call-graph for instruction
%% \args{i} under context \args{ctx}. Since \predname{Before:PointsTo}
...
%% \stdparagraph{Inter-procedural propagation of values: caller-to-callee.}
...
%% \myparagraph{Assumption:} Before execution of the rule, the points-to
%% sets encoded in relation \predname{Before:PointsTo} (for \args{base},
...
%% that the set of \args{heap} values in \pred{Before:PointsTo}{fstInstr,
%%   toCtx, ap, heap} is over-approximate, given that
%% \pred{Before:PointsTo}{i, ctx, callerAp, heap} is. Values of variable
...
%% \stdparagraph{Inter-procedural propagation of values: callee-to-caller.}
...
%% \myparagraph{Assumption:} Before execution of the rule, the points-to
%% sets encoded in relation \predname{Before:PointsTo} as well as the
%% call-graph encoded in \predname{CallGraphEdge} are
%% over-approximate.
%% %\footnote{Interestingly, we did not need the assumption
%% %that \predname{CallGraphEdge} be over-approximate for the case of
...
%% \predname{CallGraphEdge}), as long as not a single one of them is
...
In a recent article \cite{soundiness15}, Livshits et al. observe that
...
\stdparagraph{Observation: implicit over-approximation.}
...
for ``incomplete'' in past work \cite{DSA:PLDI07}).
...
\stdparagraph{Difficulty: non-monotonicity.} The first practical
...
all such objects explicitly would be highly inefficient.\footnote{The
  ...
  al. \cite{DSA:PLDI07}, which has a special form that sidesteps the
  inefficiency at the expense of imprecision. The analysis employs a
  Steensgaard/unification-based analysis
  model~\cite{mattmight:Steensgard:1996:PointsTo}. In this model, no
...
\stdparagraph{Difficulty: capturing all opaque entry points and their
...
framework~\cite{aplas/Smaragdakis15} spans hundreds of logical
...
settings~\cite{Sreedhar:2000:FIO:349299.349326}---see
...
\stdparagraph{Difficulty: wasting most work.} A sound points-to analysis
