...
%%     and Spoto~\cite{DBLP:conf/ictac/NikolicS12} report that a
...
%%     has reported similar benefits~\cite{DBLP:conf/isola/MaWD08};
...
Nikoli\'{c} and Spoto~\cite{DBLP:conf/ictac/NikolicS12} report that a
...
benefits~\cite{DBLP:conf/isola/MaWD08}.  Furthermore, results can form the base
...
Steensgaard-style \cite{steensgard:1996:PointsTo} points-to analysis.
...
meaning of the \pred{MustAlias}{i, ctx, ap1, ap2} relation is that
...
\predname{Move}, \predname{Load}, and \predname{Store}), as seen in
...
can be merged if a \predname{Move} instruction, \code{x = y}, is
encountered:
...
%That is, the new graph \emph{after} the \predname{Move} instruction will
...
If the next instruction is a \predname{Store}, \code{x.f = z}, the
previous graph will get propagated---i.e., copied. Subsequently, the \predname{Store}
...
A subsequent \predname{Load} operation, \code{z = y.g},\footnote{The
...
The \predname{Load} operation shows that our alias graph, although
...
This computes a \pred{MustAlias}{i, ctx, ap1, ap2} relation, i.e.,
...
\predname{Move} instruction, we saw the nodes of two variables getting
...
to multiple abstract objects. For instance, after a \predname{Move} instruction,
\code{x = y}, we have:
...
If the next operation is another \predname{Move}, \code{z = x}, variable
...
%% required input predicates, \prednameMay{Resolved},
%% \prednameMay{MayAlias} and \prednameMay{CallMayStoreToField}.
...
%% analysis does \emph{not} use predicate \prednameMay{Resolved} to do
%% virtual method resolution, but \emph{does} use predicates
%% \prednameMay{MayAlias} and \prednameMay{CallMayStoreToField}, in order
%% to decide when to invalidate access paths at call and store
%% instructions. 
%% %
%% Conversely, the inter-procedural$_{-\text{may}}$ analysis \emph{does}
%% use predicate \prednameMay{Resolved} to do virtual method resolution,
%% but does \emph{not} use predicates \prednameMay{MayAlias} and
%% \prednameMay{CallMayStoreToField}, instead invalidating alias
...
%% %analysis, in order to populate input tables \prednameMay{Resolved},
%% %\prednameMay{MayAlias}, and \prednameMay{CallMayStoreToField}, and not
...
analyses~\cite{repsdb,aplas/WhaleyACL05,oopsla/BravenboerS09}.
...
For instance, the \pred{Move}{i, to, from} relation represents that at
...
%% . Since the early work of Reps~\cite{repsdb}, Datalog has been
%% repeatedly used to express program analysis algorithms (e.g.,
%% \cite{aplas/WhaleyACL05,madsen13javascript,pldi/ZhangMGNY14}).
...
%% ``\pred{Head}{x,y} \rulearrow{} \pred{P}{x,z}, \pred{Q}{y,z}.''  means
%% that if values for \args{x,y,z} can be found so that \pred{P}{x,z} and
%% \pred{Q}{y,z} are simultaneously true, then \pred{Head}{x,y} is
...
%framework~\cite{vall99soot,valleerai00optimizing}, which models all
...
\hspace{-3mm}
\begin{tabular}{l l}
%\pred{Alloc}{i: I, var: V, heap: H, inMeth: M}  & \args{\# i: var = new ...} \\
...
\pred{Next}{i: I, j: I}   & \args{\# j is CFG successor of i}\\
\end{tabular}\\
%\cline{1-1}
%\hhline{=}
\\
\hspace{-3mm}
\begin{tabular}{l}
  \pred{FormalArg}{meth: M, n: $\mathbb{N}$, arg: V}\\
  \pred{ActualArg}{invo: I, n: $\mathbb{N}$, arg: V} \\ 
  \pred{FormalRet}{instr: I, meth: M, ret: V} \\
%  \pred{ActualRet}{invo: I, var: V} \\ 
  \pred{ThisVar}{meth: M, this: V} \\
  \pred{LookUp}{type: T, sig: S, meth: M} \\
  \pred{InMethod}{instr: I, meth: M} \\
  \predMay{Resolved}{var: V, type: T} \\
  \predMay{RootMethod}{meth: M} \\
\end{tabular}
\\
\hhline{=}
%\cline{1-1}
\pred{MustAlias}{inst: I, ctx: C, ap1: A, ap2: A} \\
\pred{MustCallGraphEdge}{invo: I, ctx: C, toMth: M, toCtx: C} \\
\pred{Reachable}{ctx: C, meth: M} \\
\hhline{=}
...
%% \hhline{=}
%% \begin{tabular}{llll}
%% \predMay{Resolved}{var: V, type: T} &&& \predMay{MayAlias}{var1: V, var2: V} \\
%% \predMay{CallMayStoreToField}{invo: I, fld: F} &&& \predMay{RootMethod}{meth: M} \\
%% \end{tabular}
%\\
%\cline{1-1}
\end{tabular}
%\end{center}
\caption[]{Our domain, input relations (\predname{Move}, ...),
  computed relations (\predname{MustAlias}, ...), and constructors
  (\funcname{AP}, ...). 
%% and configuration predicates (\predMay{Resolved}, ...).  
...
%handy for reference.)  The first group (starting with \predname{Move})
...
%second group (starting with \predname{MustAlias}) contains relations
...
%\prednameMay{Resolved}) also lists input relations, but of a different
...
\paragraph{Input Relations.}
  ...
  For instance, the \predname{Move} relation represents instructions
  that assign a local variable to another. There are similar input
  relations for other instruction types (\predname{Load},
  \predname{Store}, and \predname{Call}, for reading/writing heap 
  object fields and for virtual calls, respectively). The \predname{Phi} relation
  captures $\phi$ instructions, for the SSA form of our intermediate
  language. The \predname{Next} relation expresses directed edges in
  the control-flow graph (CFG): \pred{Next}{i,j} means that \args{i}
...
%  These are mostly straightforward. 
  For instance, \predname{FormalArg} shows which variable is a formal
  ...
  to the third. \predname{ActualArg} is similar, but at a method
  invocation site.  \predname{FormalRet} combines information on the
...
%  \predname{ActualRet} is a function of its first argument (a method
%  invocation site) and returns the local variable at the call site
  %  that receives the method call's return value.
  \predname{ThisVar}
  returns the \code{this} variable of a method. \predname{LookUp} is a
  ...
  \predname{InMethod} is a function from instructions to their
  containing methods. \predname{Resolved} is a predicate that
  ...
  Finally, \predname{RootMethod} is a predicate over methods, used to
...
\paragraph{Computed Relations.}
  ...
  must-alias analysis. The first relation, \predname{MustAlias}, is
  also the main output of the analysis. The relation is defined on
  access paths, i.e., expressions of the form
  ``\args{var}(\args{.fld})*''.  The meaning of \pred{MustAlias}{i,
...
%%   Section~\ref{sec:configurability}.
  ...
  analysis. \predname{MustCallGraphEdge} holds information for
  fully-resolved virtual calls: invocation site \args{invo} will call
  method \args{toMth} under the given contexts.  \predname{Reachable}
...
%%   Predicates \predname{Resolved}, \predname{MayAlias}, and
%%   \predname{CallMayStoreToField} are expected to be computed by a
%%   \emph{may-point-to} analysis, running as a preprocessing step.
%%   \predname{Resolved} holds variables that are determined to only
%%   point to objects with a unique dynamic type. \predname{MayAlias}
%%   reflects whether two variables (in the same method) may point to the
%%   same object. \predname{CallMayStoreToField} does a transitive search
...
\item We use the shorthand \predname{P*} for the reflexive, symmetric,
  transitive closure of relation \predname{P}, which is assumed to be
  ...
  rule. Specifically, \pred{MustAlias*}{i, ctx, \_, \_} denotes the
  reflexive, symmetric, transitive closure of relation
  \predname{MustAlias} with respect to its last two variables.
...
\item We introduce $\forall$: syntactic sugar that hides a Datalog
  ...
  \pred{P}{i} \rightrulearrow{} \pred{Q}{i,...}''  is true if
  \pred{Q}{i,...} holds for all \args{i} for which \pred{P}{i}
  ...
  \pred{P}{i,j,k} \rightrulearrow{} \pred{Q}{i,j,k,l}'' is interpreted
...
\begin{tabular}{l}
%\cline{1-1}
\pred{Reachable}{ctx,m} \rulearrow{} \predMay{RootMethod}{m}, \args{ctx} = \funcname{All}.\\
\rule{-2pt}{3ex}

\pred{MustAlias}{i, ctx, \funcapp{AP}{from}, \funcapp{AP}{to}} \rulearrow{} \\
\tab \pred{Move}{i, to, from}, \pred{InMethod}{i, m}, \pred{Reachable}{ctx,m}. \\
\rule{-2pt}{3ex}

\pred{MustAlias}{i, ctx, ap, \funcname{AP}(\args{to})} \rulearrow{} \\
\tab ($\forall$\args{from}: \pred{Phi}{i, to, \ldots, from, \ldots} \rightrulearrow{} \\
\tab \tab \tab              \pred{MustAlias}{i, ctx, \funcapp{AP}{from}, ap}),\\
\tab \pred{InMethod}{i, m}, \pred{Reachable}{ctx,m}. \\
\rule{-2pt}{3ex}

\pred{MustAlias}{i, ctx, \funcapp{AP}{to}, \funcname{AP}(\args{base.fld})} \rulearrow{} \\
\tab \pred{Load}{i, to, base, fld}, \\
\tab \pred{InMethod}{i, m}, \pred{Reachable}{ctx,m}. \\
\rule{-2pt}{3ex}

\pred{MustAlias}{i, ctx, \funcapp{AP}{from}, \funcname{AP}(\args{base.fld})} \rulearrow{} \\
\tab \pred{Store}{i, base, fld, from}, \\
\tab \pred{InMethod}{i, m}, \pred{Reachable}{ctx,m}. \\
\rule{-2pt}{3ex}

\pred{MustAlias}{i, ctx, \_, \_} \rulearrow{} \pred{MustAlias*}{i, ctx, \_, \_}. \\

\hhline{=}
%\rule{-2pt}{3ex}

\pred{MustCallGraphEdge}{i, ctx, toMth, toCtx}, \\
\pred{Reachable}{toCtx, toMth} \rulearrow{} \\
\tab \pred{Call}{i, base, sig}, \\
\tab \pred{InMethod}{i, m}, \pred{Reachable}{ctx,m}, \\ 
\tab \predMay{Resolved}{base, type}, \pred{LookUp}{type, sig, toMth}, \\
\tab \cons{NewContext}{i,ctx}{toCtx}. \\
\rule{-2pt}{3ex}


\pred{MustAlias}{firstInstr, toCtx, ap1, ap2} \rulearrow{} \\
\tab \pred{MustCallGraphEdge}{i, ctx, toMth, toCtx}, \\
\tab \pred{InMethod}{firstInstr, toMth}, 
     ($\forall$\args{k} \rightrulearrow{} !\pred{Next}{k, firstInstr}), \\
\tab ((\pred{FormalArg}{toMth, n, toVar}, \pred{ActualArg}{i, n, var}); \\
\tab \tab (\pred{ThisVar}{toMth, toVar}, \pred{Call}{i, var, \_})), \\
\tab \cons{PrimeAP}{\funcapp{AP}{var}}{ap1}, \cons{AP}{toVar}{ap2}. \\
\rule{-2pt}{3ex}
...
\tab \tab \tab           \pred{MustAlias}{j, ctx, callerAp1, callerAp2}),\\
\tab \cons{PrimeAP}{callerAp1}{ap1}, \cons{PrimeAP}{callerAp2}{ap2}.\\
\rule{-2pt}{3ex}

\pred{MustAlias}{i, ctx, ap1, ap2} \rulearrow{} \\
\tab \pred{MustCallGraphEdge}{i, ctx, toMth, toCtx}, \\
\tab \pred{FormalRet}{reti, toMth, \_}, \\
\tab (\pred{MustAlias}{reti, toCtx, calleeAp1, calleeAp2}; \\
\tab \tab \pred{MustAlias}{reti, \funcname{All}, calleeAp1, calleeAp2}), \\
\tab \cons{UnprimeAP}{calleeAp1}{ap1}, \\
\tab \cons{UnprimeAP}{calleeAp2}{ap2}.\\
\hhline{=}

%\cline{1-1}
%\pred{Reachable}{ctx, meth}, \pred{Reachable}{toCtx, toMeth} \rulearrow{} \\
%\tab \pred{MustCallGraphEdge}{i, ctx, toMeth, toCtx}, \\
%\tab \pred{InMethod}{i, meth}. \\
%\rule{-2pt}{3ex}

%\pred{MustAlias}{i, ctx, ap1, ap2} \rulearrow{} \\
%\tab \pred{MustAlias}{i, \funcname{All}, ap1, ap2}, \pred{InMethod}{i, meth}, \pred{Reachable}{ctx, meth}. \\
%\rule{-2pt}{3ex}

\pred{MustAlias}{i, ctx, ap3, ap4} \rulearrow{} \\
...
%\pred{MustAlias}{i, ctx, ap1, ap2} \rulearrow{} \\
%\tab \pred{Call}{i, \_, \_}, 
%     ($\forall$\args{j}: \pred{Next}{j, i} \rightrulearrow{} 
%      \pred{MustAlias}{j, ctx, ap1, ap2}), \\
...
%% %\cline{1-1}
%% \pred{Reachable}{ctx, meth}, \pred{Reachable}{toCtx, toMeth} \rulearrow{} \\
%% \tab \pred{MustCallGraphEdge}{i, ctx, toMeth, toCtx}, \pred{InMethod}{i, meth}. \\
%% \rule{-2pt}{3ex}
...
%% \pred{MustAlias}{i, ctx, ap3, ap4} \rulearrow{} \\
%% \tab \pred{MustAlias}{i, ctx, ap1, ap2}, \cons{AP}{ap1.fld}{ap3}, \cons{AP}{ap2.fld}{ap4}. \\
%% \hhline{=}
%% %\cline{1-1}
%% \pred{MustAlias}{i, ctx, ap1, ap2} \rulearrow{} \\
%% \tab !\pred{Store}{i, \_, \_, \_}, !\pred{Call}{i, \_, \_},
...
%% %% \pred{MustAlias}{i, ctx, ap1, ap2} \rulearrow{} \\
%% %% \tab \pred{Call}{i, \_, \_}, ($\forall$\args{j}: \pred{Next}{j, i} \rightrulearrow{}
%% %%                                                  \pred{MustAlias}{j, ctx, ap1, ap2}), \\
%% %% \tab ($\forall$\args{fld}: (\cons{AP}{\_.fld.\_}{ap1}; \cons{AP}{\_.fld.\_}{ap2}) \rightrulearrow{} 
%% %%       !\predMay{CallMayStoreToField}{i, fld}).\\
%% %% \rule{-2pt}{3ex}

%% %% \pred{MustAlias}{i, ctx, ap1, ap2} \rulearrow{} \\
%% %% \tab \pred{Store}{i, base, fld, \_}, ($\forall$\args{j}: \pred{Next}{j, i} \rightrulearrow{}
%% %%                                            \pred{MustAlias}{j, ctx, ap1, ap2}), \\
%% %% \tab (!\cons{AP}{\_.fld.\_}{ap1}; (\cons{AP}{var1.fld}{ap1}, !\predMay{MayAlias}{var1, base})), \\
%% %% \tab (!\cons{AP}{\_.fld.\_}{ap2}; (\cons{AP}{var2.fld}{ap2}, !\predMay{MayAlias}{var2, base})). \\
%% %\hhline{=}
%% %\cline{1-1}
%% \end{tabular}
%% %\end{center}
...
\predname{RootMethod}. This predicate designates methods that are
...
The next four \predname{MustAlias} rules handle one instruction kind
each: \predname{Move}, \predname{Phi}, \predname{Load}, and
\predname{Store}. The \predname{Move} rule merely establishes an
aliasing relationship between the two assigned variables, at the point
of the move instruction. The \predname{Phi} rule promotes aliasing
relationships that hold for all the right-hand sides of a $\phi$
instruction to its left hand side. The \predname{Load} and
\predname{Store} rules establish aliases between the loaded/stored
expression, \args{base.fld}, and the local variable used.  The last
\predname{MustAlias} rule makes the \predname{MustAlias} relation 
...
of \predname{Call}. At a \predname{Call} instruction, for method signature
\args{sig} over object \args{base}, if \args{base} has a unique (resolved)
type, then the method is looked up in that type, a \predname{MustCallGraphEdge}
is inferred from the invocation instruction to the target method and the method
is also marked as \predname{Reachable} with a callee context computed using
...
predecessors: $\forall$\args{k} \rightrulearrow{} !\pred{Next}{k, firstInstr}.
...
%context can become stronger. \predname{MustAlias} facts that were
...
%% configuration predicate \predname{CallMayStoreToField}, supplied by a
%% prior may-analysis.
...
%% to a callee, yet qualify such \predname{MustAlias} facts with the
...
%% instance, \predname{MustAlias} inferences can be combined with
...
%%   sensitivity} \cite{popl/SmaragdakisBL11}. Our \funcname{NewContext}
...
%%      ($\forall$\args{k} \rightrulearrow{} !\pred{Next}{k, firstInstr}), \\
%% \tab ($\forall$\args{j}: \pred{Next}{j, i} \rightrulearrow{} 
%%        \pred{MustAlias}{j, ctx, callerAp1, callerAp2}), \\
%% \tab \pred{RebaseAtCall}{i, ctx, var1, toVar1}, \cons{RebaseAP}{callerAp1, var1, toVar1}{ap1}, \\
%% \tab \pred{RebaseAtCall}{i, ctx, var2, toVar2}, \cons{RebaseAP}{callerAp2, var2, toVar2}{ap2}. \\
%% \end{rules}

%% In the above, all \predname{MustAlias} inferences from (all
...
%% \begin{rules}
%% \pred{MustAlias}{firstInstr, toCtx, ap1, ap2} \rulearrow{} \\
%% \tab ($\forall$\args{i, ctx}: \pred{MustCallGraphEdge}{i, ctx, toMth, toCtx} \rightrulearrow{} \\
%% \tab \tab \pred{InMethod}{firstInstr, toMth}, 
%%        ($\forall$\args{k} \rightrulearrow{} !\pred{Next}{k, firstInstr}), \\
%% \tab \tab ($\forall$\args{j}: \pred{Next}{j, i} \rightrulearrow{}
%%               \pred{MustAlias}{j, ctx, callerAp1, callerAp2}), \\
%% \tab \tab \pred{RebaseAtCall}{i, ctx, var1, toVar1}, \cons{RebaseAP}{callerAp1, var1, toVar1}{ap1}, \\
%% \tab \tab \pred{RebaseAtCall}{i, ctx, var2, toVar2}, \cons{RebaseAP}{callerAp2, var2, toVar2}{ap2}). \\
%% \end{rules}
...
%% %  (\predname{MustCallGraphEdge}) is used in the guard of a $\forall$.
...
%% %  establish a \predname{MustAlias} fact, we need to have established
%% %  a related fact for all sites that have a \predname{MustCallGraphEdge}
%% %  to the original. 


%% The requirement that \funcapp{NewContext}{i,ctx} produce contexts that
...
%% \begin{rules}
%% \cons{AP}{ap2.fld}{newAp} \rulearrow{} \pred{MustAlias}{i, ctx, ap1, ap2}, \cons{AP}{ap1.fld}{\_}. \\
%% \end{rules}
