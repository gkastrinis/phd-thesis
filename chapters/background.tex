...
An equivalent mental model is that of two instances of method \code{foo} being analyzed---\codeRAW{foo\_COPY1} and \codeRAW{foo\_COPY2}. The invocation in line 10 leads to \codeRAW{foo\_COPY1}, whereas the one in line 12 leads to \codeRAW{foo\_COPY2}. After the analysis has concluded, if one is to query regarding method \code{foo}, information from both instances will be collapsed into as single answer-set. Therefore, following the reasoning of Section~\ref{sec:back:insens}, the analysis will infer, among others, that \vpt{foo\_COPY1::arg}{o1} and \vpt{foo\_COPY2::arg}{o2}, and as a result that \vpt{obj3}{o1} and \vpt{obj4}{o2}. As already mentioned, an implicit inference is that of \vpt{arg}{o1, o2}, regarding the unqualified analysis of method \code{foo}.
...
The flow-\emph{insensitive} analysis of \ref{sec:back:flowSensitivity} will report quite different results when analyzing the SSA-form analogue of the example code (given on the side): ``\codeRAW{x\_1} has the value \code{1}'', ``\code{y} has the value \code{1}'', and ``\codeRAW{x\_2} has the value \code{2}''. The analysis is still examining instructions without taking order into account, and is unable to answer questions such as ``what is the value of variable \code{x} at the end of the method'' (whether the value of \codeRAW{x\_1} or \codeRAW{x\_2} is the final one), but nevertheless it has managed to reclaim some of the precision that was previously lost---e.g., regarding the value of variable \code{y}.
...
\begin{subfigure}{.47\textwidth}
    ...
    \caption{Any sound must-(pointer-/alias-)analysis (red) in relation to $All(P)$}
...
\end{datalog}
\caption[Core Datalog output relations and constructors]{The core Datalog output relations and constructors of contexts.}
